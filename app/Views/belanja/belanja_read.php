...
<div class="main-content bg-white">
    <h5 class="card-title mb-3"><i class='bx bxs-dashboard'></i><i class='bx bx-chevron-right'></i><?= $modulTitle ?><i class='bx bx-chevron-right'></i>View</h5>
    <form action="" method="post">
    	
    	<input type="hidden" name="bku_id" id="bku_id" value="<?= $belanja['bku_id'] ?>">
        
        <div class="mb-2 row">
            <label for="inputText" class="col-sm-2 col-form-label">Tanggal</label>
            <div class="col-sm-10">
              <span><?= $belanja['tanggal'] ?></span>
            </div>
        </div>
        ...
        <button onclick="window.location.href='<?= base_url($modulName) ?>';" class="col-2 btn btn-danger" type="button">Kembali</button>
...
$(document).on('click', '.potonganList', function() {
	...
	if (getpajak.length == 0) {
		...
		apn += '		<a role="button" id="btnDelPajak" data-id="' + kode + '"><i class="fas fa-trash" style="color:#007bff;"></i></a>&nbsp;&nbsp;';
